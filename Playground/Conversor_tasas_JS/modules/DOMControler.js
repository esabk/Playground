...
export function sugerencias(element,input,sugerenciasData,hide,tasaSeleccion){
    element.innerHTML = '';
    ...
    sugerenciasData.forEach(function(item) {
         ...
         let elementChild = newElement("div",input.value.replace("%","") +"%"+" "+ item,'sugerencia-item',"");
         elementChild.onclick=function() {
             ...
             for (let i = 0; i < tasaSeleccion.options.length; i++) {
              ...
              //Las compara con la sugerencia seleccionada y selleciona la tasa correspondiente de la lista desplegable.
